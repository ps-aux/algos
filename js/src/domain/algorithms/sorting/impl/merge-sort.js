...
const doMergeSort = (
    ...
    const len = h - l
    if (len < 2) return // _Array of size 0 & 1 => Sorted already
...
const mergeArrays = (array, aux, l, m, h) => {
    for (let i = l; i < h; i++) aux[i] = array[i]

    let i = l
    let j = m

    const moveFirst = k => (array[k] = aux[i++])

    const moveSecond = k => (array[k] = aux[j++])
    ...
    while (!firstDone || !secondDone) {
        if (firstDone) moveSecond(k)
        else if (secondDone) moveFirst(k)
        else if (aux[i] < aux[j]) moveFirst(k)
        else moveSecond(k)
        k++
