...
const switchAnimation = (nodesSel, onEnd) => {
    ...
    const moved = nodesSel.filter(d => d.move)
...
const moveLNode = (n, dst) => (n.move = pick(['x', 'y', 'value'], dst))

const node = ({ nodes }, path) =>
    path.reduce((a, v) => a.children[v], { children: [nodes[0]] }) // nodes[0] is the root node
...
const createNewNodes = entered => {
    const newNodes = entered.append('g').classed('node', true)

    newNodes.append('circle').attrs({
        r: 10
    })

    newNodes.append('text').text(prop('value'))
